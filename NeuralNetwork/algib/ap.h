...
p_next          next block in the stack unwinding vector.
                NULL means that this block is not in the vector
...
typedef struct ae_shared_pool
{
    ...
    /*
     * vector of recycled OBJECTS:
     * 1. entries in this vector store pointers to recycled objects
     * 2. every time we retrieve object, we retrieve first entry from this vector,
     *    move it to recycled_entries and return its obj field to caller/
     */
    ae_shared_pool_entry    * volatile recycled_objects;
    
    /* 
     * vector of recycled ENTRIES:
     * 1. this vector holds entries which are not used to store recycled objects;
     *    every time recycled object is retrieved, its entry is moved to this vector.
     * 2. every time object is recycled, we try to fetch entry for him from this vector
...
bool readstrings(std::string file, std::vector<std::string> *pOutput);
bool readstrings(std::string file, std::vector<std::string> *pOutput, std::string comment);
